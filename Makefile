...
RELEASE_DIR  = $(CURDIR)/_build/default/rel/cfg
VERSION     := $(shell erl -noshell -eval 'io:format("~s", [case file:consult("$(CURDIR)/src/cfg.app.src") of {ok, [{_, _, Terms}]} -> proplists:get_value(vsn, Terms, "0.0.0"); _ -> "0.0.0" end]),erlang:halt().')
RELEASE_NAME = cfg-$(VERSION)
NAME_UPPER  := $(shell echo cfg | awk '{print toupper($$1)}')
...
compile: remove-beams
	...
	$(PRE) \
            ...
            $(REBAR) compile \
        $(POST)
	$(PRE) cp -r $(CURDIR)/_build/default/lib/cfg/ebin $(CURDIR)
...
shell: shell-compile
	...
	$(PRE) erl -pa `ls -d _build/default/lib/*/ebin` \
                   ...
                   -eval "begin application:load('cfg'), code:load_file('cfg'), case application:get_all_key('cfg') of {ok, K} -> lists:foreach(fun code:load_file/1, proplists:get_value(modules, K, [])); _ -> ok end end" \
                   +B
